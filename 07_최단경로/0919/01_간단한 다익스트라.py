# 230919
# O(V^2) 시간복잡도
# 단계마다 방문하지 않은 노드 중 최단 거리가 가장 짧은 노드 선택 => 1차원 리스트 모든 원소 순차탐색

import sys
input = sys.stdin.readline      # input()보다 더 빠르게 동작하여 데이터 수가 많을 때 유리한 내장함수
...
# (n+1만큼 할당 => 노드 번호를 인덱스로 하여 바로 리스트 접근 가능하도록)
